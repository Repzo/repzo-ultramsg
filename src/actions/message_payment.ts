...
export const message_payment = async (event: EVENT, options: Config) => {
  const repzo = new Repzo(options.data?.repzoApiKey, { env: options.env });
  const action_sync_id: string = event?.headers?.action_sync_id || uuid();
  const actionLog = new Repzo.ActionLogs(repzo, action_sync_id);
  let body: Service.Payment.PaymentSchema;
  ...
  try {
    ...
    if (options.data.payments.recipientType === "Cell Phone") {
    ...
    if (options.data.payments.recipientType === "Phone") {
    ...
    const inputString: string = options.data.payments.message;
    const replacements = [
      {
        key: "totalAmount",
        value: `${parseInt(body.amount.toString()) / 1000}`,
      },
    ];
