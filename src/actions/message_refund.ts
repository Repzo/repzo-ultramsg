...
export const message_refund = async (event: EVENT, options: Config) => {
  const repzo = new Repzo(options.data?.repzoApiKey, { env: options.env });
  const action_sync_id: string = event?.headers?.action_sync_id || uuid();
  const actionLog = new Repzo.ActionLogs(repzo, action_sync_id);
  let body: Service.Refund.RefundSchema;
  ...
  try {
    ...
    if (options.data.refunds.recipientType === "Cell Phone") {
    ...
    if (options.data.refunds.recipientType === "Phone") {
    ...
    const inputString: string = options.data.refunds.message;
    const replacements = [
      {
        key: "totalAmount",
